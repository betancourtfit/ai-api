...
Last activity: 2026-07-25 -- Completed quick task 260724-mv1: the 466 MB whisper model left the Docker image for a /models volume, provisioned at boot with sha256 verification and a non-fatal failure mode; CI proves the cold-download, warm-cache and degraded boot paths
...
**Next action:** Milestone v2.0 has no phases remaining — `/gsd-complete-milestone` to archive, or `/gsd-new-milestone` to open the next one. One step left in the agreed deploy roadmap: publish the image to GHCR tagged by SHA, switch EasyPanel from build-from-git to deploy-image, trigger it from CI, and reuse the smoke assertions from 260724-shs and 260724-mv1 as the post-deploy check. Open questions to settle first, from the adversarial review: declare `permissions` per job rather than workflow-wide; decide the GHCR package visibility and, if private, create the `read:packages` PAT and configure it in EasyPanel *before* the first image push or the first deploy fails on the pull; and note that "roll back to the previous SHA tag" only rolls back the image, since EasyPanel env/config is not versioned in git. Deferred by agreement: a nightly contract test against real Cerebras/Groq (the 119 tests are 100% mocks and the 429/cooldown path has never run against a live provider), versioning EasyPanel config so rollback covers env and not just the image, and wiring `actionlint` (a `${{ }}` inside a shell comment rejected the whole workflow file and neither `Bun.YAML.parse` nor `bash -n` could see it).
**Resume file:** None
