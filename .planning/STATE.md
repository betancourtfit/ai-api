...
**Then:** Quick task 260724-p04 wired the CI minimum — `bunx tsc --noEmit` 6 errors → 0 (one production line: a `FormData` annotation in `adapters/inbound/http/routes/transcriptions.ts`), a `typecheck` script, and `.github/workflows/ci.yml` running install → typecheck → test → tracked-secret-env guard. Green on two real Actions runs on master. Not yet done: branch protection (the check runs but does not block).
