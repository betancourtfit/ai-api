...
Last activity: 2026-07-24 -- Completed quick task 260724-shs: CI now boots the real container and asserts /health == SHA, /ready, and a real transcription -- the first time this project's image has ever been started by anything
...
**Next action:** Milestone v2.0 has no phases remaining — `/gsd-complete-milestone` to archive, or `/gsd-new-milestone` to open the next one. The agreed deploy roadmap has two steps left: (1) move the 466 MB whisper model out of the image into a volume — the GHA cache is at 2.15 GB after 3 builds with four 426 MB model blobs and a 10 GB LRU ceiling, so this unblocks the cache, cheapens the pull on the mini PC, and is what makes rollback actually fast; then (2) publish to GHCR tagged by SHA, switch EasyPanel from build-from-git to deploy-image, and reuse the smoke assertions from 260724-shs as the post-deploy check. Deferred by agreement: a nightly contract test against real Cerebras/Groq (the 119 tests are 100% mocks and the 429/cooldown path has never run against a live provider), versioning EasyPanel config so rollback covers env and not just the image, and wiring `actionlint` (a `${{ }}` inside a shell comment rejected the whole workflow file and neither `Bun.YAML.parse` nor `bash -n` could see it).
**Resume file:** None
